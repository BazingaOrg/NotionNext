...
export default function PostAdjacent({ prev, next }) {
  ...
  return (
    <div id='article-end'>
      ...
      <div
        id='pc-next-post'
        className={`${isShow ? 'opacity-100' : '-mb-24 opacity-0'} hidden md:block fixed z-40 right-3 bottom-5 duration-200 transition-all`}>
        <Link
          href={`/${next.slug}`}
          className='text-sm block p-4 w-64 h-28 cursor-pointer drop-shadow-xl duration transition-all dark:bg-[#1e1e1e] border dark:border-gray-600 bg-white dark:text-gray-300 dark:hover:text-customPurple-600 hover:font-bold hover:text-customYellow-600 rounded-lg'>
