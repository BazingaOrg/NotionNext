...
const Header = props => {
  ...
  return (
    <>
      ...
      <nav
        id='nav'
        className={`z-20 h-20 py-4 top-0 w-full duration-300 transition-all
            ${fixedNav ? 'fixed' : 'relative bg-transparent'} 
            ${textWhite ? 'text-white ' : 'text-black dark:text-white'}  
            ${navBgWhite ? 'bg-white/70 dark:bg-[#18171d]/80 backdrop-filter backdrop-blur-md shadow' : 'bg-transparent'}`}>
          ...
          <div
            ...
            <div
              ...
              <h1 className='font-bold text-center text-light-400 dark:text-gray-400'>
                {props?.category || props?.post?.title || siteConfig('BIO')}
              </h1>
            </div>
          </div>
